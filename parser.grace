    ...
    object {
        ...
        method step(character) {
            ...
            if (value == "match") then { 
                ...
                print "NFA step, value, character: {value}, {character}"
                returnList := addState( out1 )
                print "end NFA step for value match"
            } else {
                returnList := list [ ]
            }
            returnList
        ...
        method addState(state) {
            print "addState: {state.asDebugString}"
            var returnList := list [ ]
            if (state.value == "split") then {
                print "splitting on {out1.asDebugString}, {out2.asDebugString}"
                print "I am {asDebugString}"
                returnList := addState(state.out1) ++ addState(state.out2)
            } elseif {state.value == "match"} then {
                print "addSate: {state.asDebugString}, {state.value}, matched match"
                returnList := list [ state ]
            } else {
                returnList := list [ state ]
            }
            print "returning addState {state.asDebugString}"
            returnList
        }
    }
}
    ...
    object {
        ...
        method patch( stateIn ) {
            ...
            while { leaves.isEmpty.not } do {
                ...
                if (stateIn.value == "match") then {
                    print "patching {item.value} to match"
                }
                item.patch( stateIn )
...
method DFAState( NFAstates_in ) {
    object {
        ...
        method step(character) {
            var newList := list [ ]
            NFAstates.do { s →
                print "DFA step: {s.asDebugString}, {character}"
                newList := newList ++ s.step(character)
                print "DFA step list: {newList}"
            }
            DFAState(newList)
        }
...
//print "DEBUG: constructed DFA: {currentState.asDebugString}"
...
currentState := currentState.step("d")
//currentState := currentState.step("a") // should now be in states {3} = {b}
//currentState := currentState.step("b")
